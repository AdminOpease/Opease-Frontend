...
export default function SubmitDvlaCodeScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmed) {
      Alert.alert("Required", "Please enter your DVLA Check Code.");
      return;
    }
    try {
      setSubmitting(true);
      await driverActions.submitDvlaCode(trimmed);
      await refreshProfile();
      Alert.alert("Submitted", "Your DVLA Check Code has been submitted successfully.");
      router.back();
    } catch (e: any) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.secondary }} contentContainerStyle={{ padding: 16 }}>
      <View style={styles.card}>
        <Text style={styles.title}>Submit DVLA Check Code</Text>
        <Text style={styles.desc}>
          Your DVLA check is due. Please enter the check code below so we can verify your driving licence.
        </Text>

        <Text style={styles.label}>DVLA Check Code</Text>
        <TextInput
          style={styles.input}
          value={code}
